...
export class PgTeamService implements ITeamRegistryService {
  ...
  async register(teamId: string, ownerId: string, pluginName: string): Promise<TeamRegistration> {
    ...
    // Check if team already exists by business teamId
    ...
    // Create agent team — DB generates UUID for id, we store business teamId separately
    await this.db.insert(agentTeams).values({
      // id: auto-generated by DB
      teamId, // business identifier (SHA-256 hash)
      orgId: orgUuid,
