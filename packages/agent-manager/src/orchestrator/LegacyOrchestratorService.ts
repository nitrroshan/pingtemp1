 ...
 *
 * The original orchestrator (PLANNER_MODE=legacy).
 * Uses dual-agent setup: orchestratorAgent + planBuilderAgent.
 * Kept for rollback. Will be removed when PLANNER_MODE=agent is stable.
 */
...
export class LegacyOrchestratorService {
  // Configuration
...
`;
  ...
  private async executeAgent(
    ...
    for await (const event of agent.execute({ message, threadId })) {
      ...
      if (LegacyOrchestratorService.STREAMING_EVENT_TYPES.has(event.type)) {
