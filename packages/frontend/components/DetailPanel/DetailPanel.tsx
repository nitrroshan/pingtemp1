...
import { X, Activity, ListTodo, FileText, ScrollText, ChevronDown, ChevronRight, Sparkles, MessageCircle, PanelRight } from 'lucide-react';
  ...
  if (selectedTask) {
    return (
      <div className="p-3 space-y-3">
        ...
        />
        <button
          onClick={() => useUiStore.getState().setDocumentPane(`crdt:${selectedTask.id}/task`)}
          className="flex items-center gap-2 w-full px-3 py-2 rounded-lg border border-border text-xs text-muted-foreground hover:text-foreground hover:bg-accent transition-colors cursor-pointer"
        >
  ...
  return (
    <div className="flex flex-col h-full">
      <div className="flex-1 overflow-y-auto">
        <PlanGroupedTasks tasks={tasks} currentPlanName={currentPlanName} onSelectTask={onSelectTask} />
      </div>
      <div className="px-3 py-2 border-t border-border shrink-0">
        <button
          onClick={() => useUiStore.getState().setDocumentPane('crdt:plan')}
          className="flex items-center gap-2 w-full px-3 py-2 rounded-lg border border-border text-xs text-muted-foreground hover:text-foreground hover:bg-accent transition-colors cursor-pointer"
        >
          <FileText size={13} className="text-primary" />
          <span>View Documents</span>
    ...
    >
      ...
      <div className="flex items-center justify-between px-4 py-2.5 border-b border-border shrink-0">
        ...
        <div className="flex items-center gap-1 shrink-0">
          <button
            onClick={() => {
              const docPath = isTaskScoped && selectedTask
                ? `crdt:${selectedTask.id}/task`
                : 'crdt:plan';
              useUiStore.getState().setDocumentPane(docPath);
            }}
