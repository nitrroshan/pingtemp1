/**
 * Socket.IO typed event contracts — backend-local copy.
 *
 * Defines `ClientToServerEvents` and `ServerToClientEvents` for typed Socket.IO.
 * Used by `SocketServerV2`, `SocketEventBroadcaster`, etc.
 *
 * Previously imported from `@ping/shared`. Inlined per v5.0 contract decision
 * so backend has no cross-package type dependency.
 */

import type { StreamPayload } from "./streamTypes.js";
