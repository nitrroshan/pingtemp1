...
> **⚠️ Misalignment Note (April 24, 2026):**  
> This planning doc references `MemoryRepo.ts` as a **git-based** backend for agent memory.  
> The [feature architecture](feature_architecture.md) was updated to use **CRDT** for memory instead  
> (see "Why memory moved from git to CRDT" section). All `MemoryRepo` references below  
> should be rewritten as CRDT-backed memory when implementation begins.  
> **Workspace repo remains git-based** — only memory changed to CRDT.
